...
public class Tokenizer {
	public static final Pattern DEFAULT_NUMBER_PATTERN = Pattern
			.compile("\\G\\s*(\\d+(\\.\\d*)?|\\.\\d+)([eE][+-]?\\d+)?");
	...
	public Tokenizer(final Scanner scanner, final Iterable<String> symbols, final String... additionalSymbols) {
		this.scanner = scanner;
		final StringBuilder sb = new StringBuilder("\\G\\s*(");

		final TreeSet<String> sorted = new TreeSet<>(new Comparator<String>() {
			@Override
			public int compare(final String s1, final String s2) {
				final int dl = -Integer.compare(s1.length(), s2.length());
	...
	public TokenType consume(final String expected, final String errorMessage) {
	...
	public TokenType consume(final String expected) {
	...
	public String consumeIdentifier(final String errorMessage) {
		if (currentType != TokenType.IDENTIFIER) {
			throw exception(errorMessage, null);
		}
		final String identifier = currentValue;
		nextToken();
		return identifier;
	}

	public ParsingException exception(final String message, final Exception cause) {
		return new ParsingException(currentPosition, currentPosition + currentValue.length(), message, cause);
	}
	...
	public String consumeComments() {
		final String result = skippedComments.toString();
	...
	public TokenType nextToken() {
		...
		if (value.length() > 0 && value.charAt(0) <= ' ') {
			...
			int pos = 0;
			while (true) {
				final int j = leadingWhitespace.indexOf('\n', pos);
				if (j == -1) {
					break;
				}
				pos = j + 1;
	...
	public boolean tryConsume(final String value) {
