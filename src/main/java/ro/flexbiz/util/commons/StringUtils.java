...
public class StringUtils {
	...
	public enum TextFilterMethod {
		CONTAINS("*=*", "Contains",
				"Find all items that contain this value. Use ''and'' and ''or'' keywords to search for several values at the same time."),
		NOT_CONTAINS("!*=*", "Not Contains",
				"Find all items without the containing value. Use ''and'' and ''or'' keywords to exclude several values."),
	...
	public static String sanitizePhoneNumber(final String phoneNumber) {
		...
		final String digitsOnly = trimmed.replaceAll("\\D", EMPTY_STRING);
	...
	/**
	 * processForFiltering is applied to text and searchString, meaning accents are
	 * removed
	 * 
	 * @param text         the text in which you want to search; if this one is
	 *                     empty, and searchString is NOT
	 *                     empty, then we return true on negative searched, or false
	 *                     otherwise
	 * @param searchString the string that is looked for in the text; if this one is
	 *                     empty,
	...
	public static boolean globalIsMatch(final String text, final String searchString,
		...
		// By this time we know the search string is not empty, so we only have a match
	...
	private static boolean smartSearch(final String text, final String searchPattern) {
		// By this time we know the search string is not empty, so an empty text is for
		// sure no match:
		if (isEmpty(text))
			return false;
	 ...
	 * 
	 * <p>
	 * For 10 million iterations it takes 15 seconds
	 * </p>
	 * <p>
	 * For 1 million iterations it takes 2 seconds
	 * </p>
	 * <p>
	 * For 10 million iterations without accent striping it takes 1 second
	 * </p>
	 */
	 ...
	 * 
	 * <p>
	 * For 1 milion iterations it takes 1.5 seconds
	 * </p>
	 */
	...
	/**
	 * <b>This is a very "expensive" method. For 1 million iterations it takes 20
	 * seconds. Be careful when you used it.</b>
	 * 
	 * <p>
	 * ggrec, 2014.12.12: Taken from Apache Commons
	 * <p>
	 * <p>
	 ...
	 * - Also the order should not matter so Groza Danut <=> Danut Groza.<br>
	 * <br>
	 *
	 * Regex symbols:<br>
	 ...
	 * <br>
	 * 
	 * .*[jie]+o+h+a+n+[jie]+s+.* would match johannes, johanes, iohanes and
	 * iohanis<br>
	...
	public static abstract class SmartNameMatcher {
		...
		public static SmartNameMatcher create(final String searchString) {
			final String matchToken = isEmpty(searchString) ? EMPTY_STRING
					: removeConsecutiveChars(processForFiltering(searchString))
							.chars()
		 ...
		 * "(.)\\1{1,}" means any character (added to group 1) followed by itself at
		 * least once<br>
		 * "$1" references contents of group 1<br>
		 * <br>
		 *
	...
	private static class SearchToken_OR extends SmartNameMatcher {
		private SearchToken_OR(final String searchString) {
			super(searchString == null ? List.of(new SearchToken_Single(EMPTY_STRING))
					: Arrays.stream(searchString.split(SPACE)).map(SearchToken_AND::new)
							.collect(Collectors.toUnmodifiableList())); // $NON-NLS-1$
		}
	...
	private static class SearchToken_AND extends SmartNameMatcher {
		private SearchToken_AND(final String searchString) {
			super(searchString == null ? List.of()
					: Arrays.stream(searchString.split(SPACE)).map(SearchToken_Single::new)
							.collect(Collectors.toUnmodifiableList())); // $NON-NLS-1$
		}
