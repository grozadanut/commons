...
public class CronExpression
{
	...
	enum CronFieldType
	{
		...
		{
			@Override
			int getValue(final ZonedDateTime dateTime)
			{
				return dateTime.getSecond();
			}

			@Override
			ZonedDateTime setValue(final ZonedDateTime dateTime, final int value)
			{
				return dateTime.withSecond(value).withNano(0);
			}

			@Override
			ZonedDateTime overflow(final ZonedDateTime dateTime)
			{
		...
		CronFieldType(final int from, final int to, final List<String> names)
		{
			this.from = from;
			this.to = to;
			this.names = names;
		 ...
		 * @return {@link ZonedDateTime} with {@code value} set for this field and all
		 *         smaller fields cleared
		 */
		abstract ZonedDateTime setValue(ZonedDateTime dateTime, int value);

		/**
		 * Handle when this field overflows and the next higher field should be
		 * incremented
		 *
		 * @param dateTime Initial {@link ZonedDateTime} instance to use
		 * @return {@link ZonedDateTime} with the next greater field incremented and all
		 *         smaller fields cleared
		 */
	...
	public ZonedDateTime nextTimeAfter(final ZonedDateTime afterTime)
	{
	...
	public LocalDateTime nextLocalDateTimeAfter(final LocalDateTime dateTime)
	{
		return nextTimeAfter(ZonedDateTime.of(dateTime, ZoneId.systemDefault())).toLocalDateTime();
	}

	public ZonedDateTime nextTimeAfter(final ZonedDateTime afterTime, final long durationInMillis)
	{
	...
	public ZonedDateTime nextTimeAfter(final ZonedDateTime afterTime, final ZonedDateTime dateTimeBarrier)
	{
		final ZonedDateTime[] nextDateTime = { afterTime.plusSeconds(1).withNano(0) };
	 ...
	 * <p>
	 * This is handled different than all other fields because there are two ways to
	 * describe the day and it is easier to handle them together in the same method.
	 *
	 ...
	 * @return {@code true} if a match was found for this field or {@code false} if
	 *         the field overflowed
	 * @see {@link SimpleField#nextMatch(ZonedDateTime[])}
	 */
	private boolean findDay(final ZonedDateTime[] dateTime, final ZonedDateTime dateTimeBarrier)
	{
		final int month = dateTime[0].getMonthValue();
	...
	private static void checkIfDateTimeBarrierIsReached(final ZonedDateTime nextTime, final ZonedDateTime dateTimeBarrier)
	{
	...
	static class FieldPart implements Comparable<FieldPart>
	{
		...
		@Override
		public int compareTo(final FieldPart o)
		{
	...
	abstract static class BasicField
	{
		...
		private BasicField(final CronFieldType fieldType, final String fieldExpr)
		{
			this.fieldType = fieldType;
			parse(fieldExpr);
		}

		private void parse(final String fieldExpr)
		{ // NOSONAR
			final String[] rangeParts = fieldExpr.split(",");
			for (final String rangePart : rangeParts)
			{
				final Matcher m = CRON_FIELD_REGEXP.matcher(rangePart);
				if (!m.matches())
				{
					throw new IllegalArgumentException(
							"Invalid cron field '" + rangePart + "' for field [" + fieldType + "]");
				}
				final String startNummer = m.group("start");
				final String modifier = m.group("mod");
				final String sluttNummer = m.group("end");
				final String incrementModifier = m.group("incmod");
				final String increment = m.group("inc");

				final FieldPart part = new FieldPart();
				part.increment = 999;
				if (startNummer != null)
				{
		...
		protected void validatePart(final FieldPart part)
		{
		...
		private void validateRange(final FieldPart part)
		{
		...
		protected int mapValue(final String value)
		{
			int idx;
		...
		protected boolean matches(final int val, final FieldPart part)
		{
		...
		protected int nextMatch(final int val, final FieldPart part)
		{
			if (val > part.to)
			{
				return -1;
			...
			final int remainder = ((nextPotential - part.from) % part.increment);
			if (remainder != 0)
			{
	...
	{
		SimpleField(final CronFieldType fieldType, final String fieldExpr)
		{
		...
		public boolean matches(final int val)
		{
			...
			{
				for (final FieldPart part : parts)
				{
					if (matches(val, part))
					{
						return true;
		...
		/**
		 * Find the next match for this field. If a match cannot be found force an
		 * overflow and increase the next greatest field.
		 *
		 * @param dateTime {@link ZonedDateTime} array so the reference can be modified
		 * @return {@code true} if a match was found for this field or {@code false} if
		 *         the field overflowed
		 */
		protected boolean nextMatch(final ZonedDateTime[] dateTime)
		{
			final int value = fieldType.getValue(dateTime[0]);

			for (final FieldPart part : parts)
			{
				final int nextMatch = nextMatch(value, part);
				if (nextMatch > -1)
				{
					if (nextMatch != value)
					{
	...
	static class DayOfWeekField extends BasicField
	{
		...
		DayOfWeekField(final String fieldExpr)
		{
			super(CronFieldType.DAY_OF_WEEK, fieldExpr);
		}

		boolean matches(final LocalDate dato)
		{
			for (final FieldPart part : parts)
			{
				if ("L".equals(part.modifier))
				{
					final YearMonth ym = YearMonth.of(dato.getYear(), dato.getMonth().getValue());
				...
				} else if ("#".equals(part.incrementModifier))
				{
					...
					{
						final int num = dato.getDayOfMonth() / 7;
	...
	{
		DayOfMonthField(final String fieldExpr)
		{
